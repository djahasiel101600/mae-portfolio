      ...
      <div className="container mx-auto px-4 py-20 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            ...
            <motion.p {...fadeInUp} className="text-xl md:text-2xl text-foreground font-medium mb-6">
              Virtual Assistant | Administrative & Data Support Specialist
            </motion.p>

            <motion.p {...fadeInUp} className="text-lg text-muted-foreground max-w-2xl mx-auto mb-8">
              Detail-oriented and reliable professional with strong experience in administrative support, data organization, and report preparation. Committed to delivering organized, accurate, and efficient remote support.
            </motion.p>

            <motion.div {...fadeInUp} className="flex flex-wrap justify-center gap-4 mb-10">
              {contactItems.map((item, index) =>
                item.href ? (
                  <a key={index} href={item.href} className="flex items-center gap-2 px-4 py-2 bg-card/70 backdrop-blur-sm rounded-full border border-border/50 hover:border-primary/50 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                    <item.icon className="w-4 h-4 text-primary" />
                    <span className="text-sm">{item.text}</span>
                  </a>
                ) : (
                  <div key={index} className="flex items-center gap-2 px-4 py-2 bg-card/70 backdrop-blur-sm rounded-full border border-border/50">
                    <item.icon className="w-4 h-4 text-primary" />
                    <span className="text-sm">{item.text}</span>
                  </div>
                )
