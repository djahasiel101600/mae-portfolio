  ...
  return (
    <section id="skills" className="py-20 bg-background" aria-labelledby="skills-heading">
      <div className="container mx-auto px-4">
        ...
        >
          <h2 id="skills-heading" className="text-4xl md:text-5xl font-bold text-primary mb-4" style={{ fontFamily: "'Playfair Display', serif" }}>
            Core Skills
          </h2>
        </motion.div>

        <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} className="hidden md:grid md:grid-cols-3 gap-6">
            ...
            <motion.div key={index} variants={itemVariants}>
              <Card className="h-full hover:scale-105 transition-transform duration-300 border-t-4 border-t-primary/30 group">
                <CardHeader>
                  ...
                  <CardTitle className="text-primary text-xl">{skill.title}</CardTitle>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="md:hidden">
          <Tabs defaultValue="admin" className="w-full">
            <TabsList className="grid w-full grid-cols-3 mb-6">
              {skills.map((skill) => (
                <TabsTrigger key={skill.id} value={skill.id} className="text-xs">
                  <skill.icon className="w-4 h-4 mr-1" />
                  <span className="hidden sm:inline">{skill.title.split(" ")[0]}</span>
                </TabsTrigger>
              ))}
            </TabsList>
